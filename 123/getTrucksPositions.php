...
$query = sprintf("select * from trucks where fecha > '".$fechaBusqueda."' and latitud<>''");
    ...
    if ($result) {
      while($row = mysql_fetch_array($result)) {
		...
		$arrayTmp= array('idTruck'=>$row['id'],'nombre'=>$row['name'],'latitud' =>$row['latitud'],'longitude'=>$row['longitud'],'latituddelta'=>'','longitudedelta'=>'','direccion'=>$row['direccion'],'lugar'=>$row['lugar'],'status'=>$row['status']);
